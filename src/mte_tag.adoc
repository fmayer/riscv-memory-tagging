...
[NOTE]
====
If software desires to read tag in these non-performance critical paths, it can
do so by creating an alternate read-only mapping of tag storage with the help
...
is not on a tagcheck-exempt code page (see <<TAGGED_PAGE>>).
